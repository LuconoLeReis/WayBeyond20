...
class Beyond20RollRenderer {
    ...
    async postDescription(request, title, source, attributes, description, attack_rolls = [], roll_info = [], damage_rolls = [], open = false) {
        ...
        postToDiscord(discordChannel ? discordChannel.secret : "", request, title, source, attributes, description, attack_rolls, roll_info, damage_rolls, total_damages, open, this._settings).then(discord_error => {
            if (discord_error != undefined)
                this._displayer.displayError("WayBeyond20 Discord Integration: " + discord_error);
        });
    ...
    wayBeyond20HandleResolvedD20(request, rolls, type) {
        if (!request || type !== "skill-check" || request.skill !== "Stealth") return;
        if (!Array.isArray(rolls) || rolls.length === 0) return;

        const activeRolls = rolls.filter(roll => roll && !roll.discarded && roll.total !== undefined && roll.total !== null);
        if (activeRolls.length === 0) return;

        // If a roll mode produces more than one active result, use the highest visible result for this tracker.
        // Advantage/disadvantage rolls are already marked discarded before this method runs.
        const total = Math.max(...activeRolls.map(roll => Number(roll.total)).filter(total => !Number.isNaN(total)));
        if (Number.isNaN(total) || total < 15) return;

        const character = request.character || {};
        const effect = {
            name: "Stealth",
            source: "Hide Action",
            duration: "Until you are found or reveal yourself",
            concentration: false,
            checkTotal: total,
            hideDc: total,
            conditions: ["Invisible"],
            notes: "Hide action succeeded against DC 15."
        };
        ...
        try {
            ...
            console.warn("WayBeyond20 could not send Hide effect update", error);
        }
    }
    ...
    async rollD20(request, title, data, type) {
        ...
        this.wayBeyond20HandleResolvedD20(request, rolls, type);
 ...
 "");
        ...
        if (request["waybeyond20-temp-hp-on-hit"] &&
            typeof this._roller.applyDamageBasedTempHP === "function") {
            const totalDamage = damage_rolls.reduce((total, damageRoll) => {
                const [, roll, flags] = damageRoll;
            ...
            if (totalDamage > 0) {
                await this._roller.applyDamageBasedTempHP(totalDamage, request);
            }
        }
    ...
    displayAvatarToDiscord(request) {
        ...
        postToDiscord(discordChannel ? discordChannel.secret : "", request, request.name, "", {}, "", [], [], [], [], false, this._settings).then((error) => {
            if (error)
                this._displayer.displayError("WayBeyond20 Discord Integration: " + error);
        });
    }
