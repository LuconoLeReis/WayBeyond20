...
class Character extends CharacterBase {
    ...
    updateFeatures() {
        let update = false;
        const parsed_feature_changes = [];
        const collectNewFeatures = (current, previous) => {
            const oldValues = new Set(Array.isArray(previous) ? previous : []);
            for (const value of (Array.isArray(current) ? current : [])) {
                if (value && !oldValues.has(value)) parsed_feature_changes.push(value);
            }
        };
        ...
        if (class_detail.length > 0) {
            ...
            const previousClassFeatures = this.getSetting("class-features", []);
            if (!isListEqual(this._class_features, previousClassFeatures)) {
                console.log("New class feature");
                collectNewFeatures(this._class_features, previousClassFeatures);
                update = true;
            }
        } else {
        ...
        if (race_detail.length > 0) {
            ...
            const previousRacialTraits = this.getSetting("racial-traits", []);
            if (!isListEqual(this._racial_traits, previousRacialTraits)) {
                console.log("New race feature");
                collectNewFeatures(this._racial_traits, previousRacialTraits);
                update = true;
            }
        } else {
        ...
        if (feats_detail.length > 0) {
            ...
            const previousFeats = this.getSetting("feats", []);
            if (!isListEqual(this._feats, previousFeats)) {
                console.log("New Feats");
                collectNewFeatures(this._feats, previousFeats);
                update = true;
            }
        } else {
        ...
        const scroll_spellcasting_changed = this._applyUseMagicDeviceScrollSpellcasting();
        if (scroll_spellcasting_changed ||
            !isObjectEqual(this._spell_modifiers, this.getSetting("spell_modifiers", {})) ||
        ...
        if (this._settings && update) {
            this.mergeCharacterSettings({
                ...
                // Keep the useful confirmation, but debounce and deduplicate it so the
                // Features & Traits render cycle cannot flood the character sheet.
                this._queueFeatureParseNotice(parsed_feature_changes);
            });
        }
    }

    _queueFeatureParseNotice(features) {
        const additions = Array.isArray(features) ? features.filter(Boolean) : [];
        if (additions.length === 0) return;
        if (!this._pending_feature_parse_notice) this._pending_feature_parse_notice = new Set();
        additions.forEach(feature => this._pending_feature_parse_notice.add(feature));
        if (this._feature_parse_notice_timer) clearTimeout(this._feature_parse_notice_timer);
        this._feature_parse_notice_timer = setTimeout(() => {
            const parsed = [...this._pending_feature_parse_notice];
            this._pending_feature_parse_notice.clear();
            const shown = parsed.slice(0, 6);
            const remaining = parsed.length - shown.length;
            const details = shown.join(", ") + (remaining > 0 ? `, and ${remaining} more` : "");
            const message = `WayBeyond20 parsed new feature information${details ? `: ${details}` : ""}.`;
    ...
    _formatSignedModifier(value) {
        const parsed = parseInt(value || 0);
        return parsed >= 0 ? `+${parsed}` : `${parsed}`;
    }

    _clearUseMagicDeviceScrollSpellcasting() {
        let changed = false;
        const scrollSpellcastingFeature = "Spellcasting: Scrolls";
        const originalFeatureCount = this._class_features.length;
        this._class_features = this._class_features.filter(feature => feature !== scrollSpellcastingFeature);
        if (this._class_features.length !== originalFeatureCount) changed = true;

        for (let spellMap of [this._spell_modifiers, this._spell_attacks, this._spell_saves]) {
            if (spellMap && Object.prototype.hasOwnProperty.call(spellMap, "Scrolls")) {
                delete spellMap["Scrolls"];
                changed = true;
            }
        }
        return changed;
    ...
    _applyUseMagicDeviceScrollSpellcasting() {
        const hasUseMagicDevice = this.hasClassFeature("Use Magic Device", true);

        if (!hasUseMagicDevice) {
            return this._clearUseMagicDeviceScrollSpellcasting();
        }

        let changed = false;
        const scrollSpellcastingFeature = "Spellcasting: Scrolls";
        if (!this.hasClassFeature(scrollSpellcastingFeature)) {
            // WayBeyond20: Use Magic Device grants a scroll-specific spellcasting path.
            // It is exposed alongside any normal class spellcasting options so scroll rolls
            // can ask once which valid path to use and remember that choice.
            this._class_features.push(scrollSpellcastingFeature);
            changed = true;
        }

        const intelligence = this.getAbility("INT") || {mod: 0};
        const proficiency = parseInt(this._proficiency || 0);
        const spellModifier = parseInt(intelligence.mod || 0);
        const spellAttack = spellModifier + proficiency;
        const spellSave = 8 + spellModifier + proficiency;

        const modifierText = this._formatSignedModifier(spellModifier);
        const attackText = this._formatSignedModifier(spellAttack);
        const saveText = `${spellSave}`;

        if (this._spell_modifiers["Scrolls"] !== modifierText) {
            this._spell_modifiers["Scrolls"] = modifierText;
            changed = true;
        }
        if (this._spell_attacks["Scrolls"] !== attackText) {
            this._spell_attacks["Scrolls"] = attackText;
            changed = true;
        }
        if (this._spell_saves["Scrolls"] !== saveText) {
            this._spell_saves["Scrolls"] = saveText;
            changed = true;
        }
    ...
    mergeCharacterSettings(data, callback = null) {
        ...
        const changedData = {};
        for (let k in data) {
            if (!valuesEqual(this._settings?.[k], data[k])) {
