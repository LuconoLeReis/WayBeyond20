...
function addRollButton(character, callback, where, { small = false, append = false, prepend = false, before = false, image = true, text = "WayBeyond20" } = {}) {
    last_character_used = character;
...
function beyond20SendMessageFailure(character, response) {
    const passiveSyncActions = ["hp-update", "conditions-update", "effects-update", "update-combat"];
    if (!response || (response.request && passiveSyncActions.includes(response.request.action)))
        return;
    ...
    if (["roll", "rendered-roll"].includes(response.request.action)  && ((response.vtt && response.vtt[0] == "dndbeyond") || response.error)) {
        ...
        alertify.error("<strong>WayBeyond20 : </strong>" + response.error);
    }
 ...
 * This allows extensions to send DOM API calls to WayBeyond20 from the ddb site itself
 */
function _sendCustomMessageToBeyond20(message) {
    ...
    if (!message || !message.action) {
        console.error("WayBeyond20: Invalid message received: ", message);
        return;
    }
    // Check for allowed actions only
    if (!["roll", "rendered-roll", "hp-update", "conditions-update", "effects-update", "update-combat"].includes(message.action)) {
        console.error("WayBeyond20: Invalid action received: ", message.action);
        return;
    }
