console.log("WayBeyond20: Roll20 module loaded.");
...
let wayBeyond20Roll20BridgeAvailable = true;

function wayBeyond20SendCustomMessageToExtension(message) {
    if (!message || !message.action) {
        console.error("WayBeyond20: Invalid Roll20 page message", message);
        return;
    }
    if (!["waybeyond20-token-binding", "waybeyond20-turn-update", "waybeyond20-damage-result"].includes(message.action)) {
        console.error("WayBeyond20: Invalid Roll20 page action", message.action);
        return;
    }
    if (!wayBeyond20Roll20BridgeAvailable) return;

    try {
        // Content scripts are the only Roll20 layer that can talk to chrome.runtime.
        // Do not check chrome.runtime.id here: in some content-script contexts it can be absent
        // even though sendMessage is usable, which caused false "bridge unavailable" errors.
        const runtime = (typeof chrome !== "undefined" && chrome.runtime) ? chrome.runtime : null;
        if (!runtime || typeof runtime.sendMessage !== "function") {
            wayBeyond20Roll20BridgeAvailable = false;
            return;
        }
        runtime.sendMessage(message, () => {
            ...
            console.debug("WayBeyond20: Unable to forward Roll20 page message", text);
        });
    } catch (err) {
        ...
        console.debug("WayBeyond20: Unable to forward Roll20 page message", err);
    }
}

function wayBeyond20SendResolvedDamageToDDB(renderedRequest) {
    const originalRequest = renderedRequest && renderedRequest.request ? renderedRequest.request : null;
    const effects = originalRequest && Array.isArray(originalRequest.effects) ? originalRequest.effects : [];
    const inferredIntent = effects.some(effect => String(effect || "").toLowerCase().includes("draining attack")) &&
        !!originalRequest?.["waybeyond20-unarmed-strike"];
    const intent = originalRequest ? (originalRequest["waybeyond20-temp-hp-on-hit"] || (inferredIntent ? {
        mode: "damage-dealt",
        source: "Draining Attack",
        attack: originalRequest.name,
        inferred: true
    } : null)) : null;

    wayBeyond20DebugLog("Roll20", "Resolved damage callback evaluated", {
        hasRenderedRequest: !!renderedRequest,
        hasOriginalRequest: !!originalRequest,
        intent,
        name: originalRequest ? originalRequest.name : null,
        effects,
        damageRollCount: renderedRequest && Array.isArray(renderedRequest.damage_rolls) ? renderedRequest.damage_rolls.length : null
    }, !!settings["waybeyond20-debug-log"]);

    if (!originalRequest || !intent || !originalRequest.character || !Array.isArray(renderedRequest.damage_rolls)) return;

    const totalDamage = renderedRequest.damage_rolls.reduce((total, damageRoll) => {
        if (!Array.isArray(damageRoll) || damageRoll.length < 3) return total;
        const roll = damageRoll[1];
        const flags = Number(damageRoll[2] || 0);
        if (!roll || typeof roll !== "object") return total;
        if (flags === DAMAGE_FLAGS.MESSAGE) return total;
        if (flags & DAMAGE_FLAGS.HEALING) return total;
        if (flags & DAMAGE_FLAGS.VERSATILE) return total;
        if (flags & DAMAGE_FLAGS.CONDITIONAL) return total;
        const amount = Number(roll.total);
        return Number.isFinite(amount) && amount > 0 ? total + amount : total;
    }, 0);

    wayBeyond20DebugLog("Roll20", "Resolved Draining Attack damage calculated", { totalDamage, intent }, !!settings["waybeyond20-debug-log"]);
    if (totalDamage <= 0) return;
    wayBeyond20SendCustomMessageToExtension({
        action: "waybeyond20-damage-result",
        character: originalRequest.character,
        totalDamage,
        source: "Roll20"
    });
}
...
function rollHitDice(request) {
    const rname = request.name || ("Hit Dice" + (request.multiclass ? `(${request.class})` : ""));
    return template(request, "simple", {
        "charname": request.character.name,
        "rname": rname,
...
async function handleOGLRenderedRoll(request) {
    ...
    wayBeyond20SendResolvedDamageToDDB(request);
...
async function handleRenderedRoll(request) {
    ...
    postChatMessage(message, request.character);
    wayBeyond20SendResolvedDamageToDDB(request);
    if (originalRequest.type === "initiative" && settings["initiative-tracker"] && originalRequest.character && originalRequest.character.name) {
        sendCustomEvent("BindSelectedToken", [originalRequest.character.name, "initiative-rendered"]);
    }
    if (rollDamages) {
        hookRollDamages(rollDamages, request);
    }
...
function handleMessage(request, sender, sendResponse) {
    ...
    if (request.action == "settings") {
        ...
        const concentration = request.concentration || null;
        ...
        let message = "";
        if (effects.length == 0) {
            message = em_command + "has no tracked spell effects";
        } else {
            const effectNames = effects.map(effect => {
                const duration = effect.duration ? `, ${effect.duration}` : "";
                const castAt = effect.castAt ? `, cast at ${effect.castAt}` : "";
                const concentrationText = effect.concentration ? " [Concentration]" : "";
                return `${effect.name}${concentrationText}${castAt}${duration}`;
            });
            message = em_command + "tracked spell effects: " + effectNames.join("; ");
            if (concentration && concentration.name) {
                message += `. Current concentration: ${concentration.name}`;
            }
        }
        postChatMessage(message, character_name);
...
addCustomEventListener("SendMessage", wayBeyond20SendCustomMessageToExtension);
